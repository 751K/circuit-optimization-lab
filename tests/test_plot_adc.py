...
def test_static_synthetic(tmp_path, plot_adc):
    path = plot_adc.plot_sar_static(
        _synthetic_sweep(), out_dir=tmp_path, note="synthetic")
    _png_ok(path)


def test_spectrum_synthetic(tmp_path, plot_adc):
    ...
    _png_ok(plot_adc.plot_sar_spectrum(result, out_dir=tmp_path))


def test_mc_synthetic_with_overflow(tmp_path, plot_adc):
    ...
    _png_ok(plot_adc.plot_sar_mc(
        {"arrays": arrays, "summary": summary}, out_dir=tmp_path))
...
def test_conversion_synthetic_no_clk(tmp_path, plot_adc):
    adc = {"sample_input": "sample", "sample_bar_input": "sample_b",
           "bit_inputs": ["b2p", "b1p", "b0p"], "comparator_node": "vout",
           "comparator_threshold": 0.5}
    _png_ok(plot_adc.plot_sar_conversion(
        _fake_conversion(with_clk=False), adc=adc, out_dir=tmp_path))


def test_conversion_missing_optional_keys(tmp_path, plot_adc):
    ...
    _png_ok(plot_adc.plot_sar_conversion(
        result, adc=None, out_dir=tmp_path, filename="degraded.png"))


# ── ngspice-guarded: real 3-bit example, end to end ───────────────────────────

@_needs_freepdk45
def test_conversion_3bit_real(tmp_path, plot_adc):
    ...
    _png_ok(plot_adc.plot_sar_conversion(conv, adc=spec.adc, out_dir=tmp_path))


@_needs_freepdk45
def test_static_3bit_real(tmp_path, plot_adc):
    from circuitopt.circuit_loader import load_circuit_json
    from circuitopt.sar import run_sar_sweep
    spec = load_circuit_json(EXAMPLE3)
    vin = (np.arange(8) + 0.5) / 8.0
    _png_ok(plot_adc.plot_sar_static(
        run_sar_sweep(spec, vin), out_dir=tmp_path))


@_needs_freepdk45
def test_spectrum_3bit_real(tmp_path, plot_adc):
    from circuitopt.circuit_loader import load_circuit_json
    ...
    _png_ok(plot_adc.plot_sar_spectrum(sig, out_dir=tmp_path))


@_needs_freepdk45
def test_mc_3bit_real(tmp_path, plot_adc):
    from circuitopt.circuit_loader import load_circuit_json
    from circuitopt.sar_mc import sar_mismatch_mc
    spec = load_circuit_json(EXAMPLE3)
    mc = sar_mismatch_mc(spec, n=2, seed=1)
    _png_ok(plot_adc.plot_sar_mc(mc, out_dir=tmp_path))
