...
- `pmos_chopper_pac(...)` 是 chopper 兼容包装器，内部调用通用
  `core.pac_solver.pac_solve(...)`。默认使用解析伴随谐波平衡内核：沿 PSS
  轨道采样周期性 G(t)/C(t)，FFT 到谐波系数，构建转换矩阵 Y_HB(f)，每频率
  一次伴随线性求解得到 sideband-0 增益 — O(1) 求解，无额外瞬态运行。
  设置 `analytic=False` 可回退到原有限差分 shooting 路径（一次 `n_state`
  个单周期瞬态的状态线性化，加上每频点 2 个输入正交有限差分周期）。设置
  `time_domain=True` 可启用 rail-driven chopper 快速路径：在时域构建一次单周期
  Floquet monodromy，然后每频点求一个小的 quasi-periodic 边界系统
  （`method="pss_time_domain"`）。bordered 或 vsource-driven 等不支持场景会在
  `analytic=True` 时回退到 HB。这条快速路径目前仍是 opt-in：2026-06-26 对入库
  Cadence chopper 参考的检查结果为 baseband typical −0.44%、slow −1.89%、
  fast −0.27%。静态 PSS 会自动退化为普通 `ac_solve` fast path，不跑 PAC transient。
...
参考，默认 HB PAC 的 baseband 和 200 Hz 增益误差均 <1%；原生 PNoise 在相同
dec=10 噪声频率网格和 `maxsideband=10` 下 IRN 误差 <1%。
...
- PMOS chopper wrapper 默认使用 Numba-grid transient 路径
  ...
  使用 1 个 stabilization 周期。这样 residual/nfail 收敛判据不变，同时避免每个
  周期退回 Python fallback 重跑。
...
- 四条性能路径，按优先级依次尝试：
  ...
  2. **Time-domain Floquet PAC**（可选，`time_domain=True`）— 在均匀轨道网格上
     采样周期性 G(t)/C(t) 和输入耦合，先构建一次与频率无关的 monodromy，
     再对每个频点求 `(exp(jωT)I - Ψ)x0 = g`。它避免 HB 边带截断和大型
     `(2K+1)n` 转换矩阵，但当前要求 rail-driven、无 bordered MNA 支路且
     Numba 线性化可用。不支持时返回 `None` 并继续尝试下一条路径。
  3. **解析伴随**（默认，`analytic=True`）— 沿 PSS 轨道采样周期性 G(t)/C(t)
...
- Numba 可用时，大规模 LPTV PNoise 会使用编译版 HB block 组装和
  ...
  计算 gm/gds，在小电流或 kink 附近自动回退原有限差分。Numba/Rust 这类编译实现
  主要能加速矩阵填充和噪声折叠循环；HB 线性求解本身主要由 BLAS/LAPACK、
  SuperLU 或 GMRES 决定，不是 Python 循环开销主导。
...
旧 UI chopper 全流程瓶颈是通用 HB PAC frequency solve：`PSS+PAC(HB)+PNoise`
61 点约 25.6s（PSS≈0.35s、PAC≈24.7s、PNoise≈0.55s），121 点约 48.9s
（PSS≈0.44s、PAC≈47.6s、PNoise≈0.93s）。启用可选 time-domain PAC 后，同一
PSS 轨道上 PAC 61 点约 1.3s、121 点约 1.9s。但由于 slow-corner baseband
相对 Cadence 仍约 −1.9%，它还不能作为默认路径。非 chopper AFE 的
`DC+AC+Noise` 121 点在复用 AC 结果时约 1.8ms。
...
- 原生 `pmos_chopper_pac` 和 `pmos_chopper_pnoise`（第一性原理，无标定常数）已
  ...
  `f_chop=200 Hz` 时默认 HB PAC baseband 和 200 Hz 增益误差均 <1%，PNoise IRN
  在相同 dec=10 噪声网格下误差 <1%。
