...
- PSS 辅助的 PAC（周期 AC），支持解析伴随谐波平衡（默认）或有限差分 shooting。
- 谐波平衡 PNoise（周期噪声），含循环平稳噪声折叠。
- 工艺角与逐器件 mismatch 扰动。
- 面向 Cadence/Spectre 的验证，涵盖工作点、AC、噪声、瞬态、PSS、PAC 和 PNoise 行为。

实现刻意保持小而自包含。目前由 `core/` 下的十八个 Python 源文件组成（另含 `__init__.py` 和 CLI 入口 `__main__.py`）。
...
core/
  ...
  device_model.py      TransistorModel ABC + NumbaParams + 模型工厂/注册表。
...
- **`register_model()` / `create_device()`** — 工厂 + 注册表。求解器文件调用 `create_device("pmos_tft", W=…, L=…)` 而非直接导入 `PMOS_TFT`。新增晶体管模型只需一个新子类 + 一次 `register_model` 调用。
...
- `pmos_chopper_lptv_analysis(...)` 用这些有限边沿谐波权重折叠 PMOS-switch
  sideband response/noise。默认会应用 UI 锁定尺寸、225 Hz、switch 5000/30
  这一组 Spectre PSS/PAC/PNoise 标定：一个小的换向相位修正和一个小的周期噪声
  PSD scale；需要查看原始 quasi-static 边带结果时，可设置 `cadence_calibrated=False`。
...
- `pmos_chopper_pac(...)` 是 chopper 兼容包装器，内部调用通用
  ...
  个单周期瞬态的状态线性化，加上每频点 2 个输入正交有限差分周期）。静态
  PSS 会自动退化为普通 `ac_solve` fast path，不跑 PAC transient。
...
参考，原生 PAC 的 baseband 和 200 Hz 增益误差均 <1%；原生 PNoise 在相同 dec=10
噪声频率网格和 `maxsideband=10` 下 IRN 误差 <1%。
...
- 三条性能路径，按优先级依次尝试：
  ...
  2. **解析伴随**（默认，`analytic=True`）— 沿 PSS 轨道采样周期性 G(t)/C(t)
  ...
  3. **有限差分 shooting**（`analytic=False`）— 有限差分状态转移矩阵 Φ 和
...
- 结果中包含 `pac_period_runs`、`pac_state_cache_hit`、`pac_input_cache_hits`
  等计数器，以及 `method` 字段（`"pss_analytic_adjoint"` 或 `"pss_fd_shooting"`）。
...
- 原生 `pmos_chopper_pac` 和 `pmos_chopper_pnoise`（第一性原理，无标定常数）已
  ...
  `f_chop=200 Hz` 时 PAC baseband 和 200 Hz 增益误差均 <1%，PNoise IRN 在相同
  dec=10 噪声网格下误差 <1%。
