...
- `pmos_chopper_pac(...)` 是 chopper 兼容包装器，内部调用通用
  ...
  设置 `time_domain=False` 可跑解析伴随 HB 对照路径；设置 `analytic=False` 可回退
  到原有限差分 shooting 路径。静态 PSS 会自动退化为普通 `ac_solve` fast path，
  不跑 PAC transient。
...
- 四条性能路径，按优先级依次尝试：
  ...
  2. **Time-domain Floquet PAC**（`time_domain=True`；chopper wrapper 默认）— 在均匀轨道网格上
     ...
     小信号状态。不支持 bordered/vsource 驱动时返回 `None` 并继续尝试下一条路径。
...
同一 PSS 轨道上 61 点约 1.4s。非 chopper AFE 的 `DC+AC+Noise` 121 点在复用
AC 结果时约 1.8ms。
