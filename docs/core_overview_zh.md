...
首次被 import 时一次性算好的常量，事后再设环境变量是静默 no-op。`core/__init__.py`
在其求解器 import（会连带把 `numba_kernels` 传递 import 进来）之前，先对 `sys.argv`
预扫 `--no-numba` 并设 `CIRCUIT_USE_NUMBA=0`——在 `python -m core …` 下这个
...
（非 CLI）里禁用 Numba，必须在 `import core` **之前**设 `CIRCUIT_USE_NUMBA=0`。
...
- `pmos_chopper_pnoise(...)` 是 chopper 兼容包装器，内部调用通用
  `core.pnoise_solver.pnoise_solve(...)`。chopper 验证默认使用 time-domain
