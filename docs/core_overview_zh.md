...
- `pmos_chopper_pss(...)` 把同一 hard-switched 八 PMOS 拓扑接入通用 shooting
  ...
  的本地工作点基础。
- `pmos_chopper_pac(...)` 是 PSS 辅助的有限差分 PAC 求解器。它直接在非线性
  瞬态引擎上有限差分状态转移矩阵 Φ 和复输入扰动，求解准周期边界方程
  `(Φ-γI)dx0=-b`，取输出包络的 DC 傅里叶系数即得 sideband-0 PAC 增益。代价
  为每频点 `n_state+2` 次单周期瞬态。
- `pmos_chopper_pnoise(...)` 是第一性原理 LPTV 周期噪声求解器，在 PSS 轨道上
  使用谐波平衡。沿轨道 N 点采样 → 时变小信号 G(t)/C(t) → FFT 到频域 →
  组装 `nb×nb` 块矩阵 `Y[kr,kc] = G_{kr-kc} + jω·C_{kr-kc}` → 每基带频率一次
  伴随求解得到传递阻抗 Z_{j,k} → 循环平稳噪声折叠 `S_out = Σ_j Σ_k |Z_{j,k}|² S_j`。
...
- shooting Jacobian 第一版通过有限差分一周期映射得到。这样实现保持 topology
  通用，并与 transient 的离散方式和器件模型完全一致。
- 返回一周期轨迹、`x0`、`x_end`、残差向量/范数、收敛标志和迭代历史。

这一步输出的 PSS 周期轨道可以直接供 `pmos_chopper_pac` 和
`pmos_chopper_pnoise` 使用，无需重复求解。
