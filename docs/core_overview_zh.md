...
- `pmos_chopper_pac(...)` 是 chopper 兼容包装器，内部调用通用
  `core.pac_solver.pac_solve(...)`。通用 PAC 在 PSS 轨道附近有限差分状态转移矩阵
  Φ 和复输入扰动，求解准周期边界方程 `(Φ-γI)dx0=-b`，取输出包络的 DC
  傅里叶系数即得 sideband-0 增益。真正 LPTV 时，代价为一次 `n_state` 个
  单周期瞬态的状态线性化，加上每频点 2 个输入正交有限差分周期。静态 PSS
  会自动退化为普通 `ac_solve` fast path，不跑 PAC transient。
...
Spectre `tran` 对齐，原生 PNoise IRN 在全分辨率下与 Spectre PNoise 偏差约 6% 以内
（见 `test_pmos_chopper_pnoise_matches_cadence_band`）。
...
- 第一轮 shooting Jacobian 通过有限差分一周期映射得到；后续默认用 Broyden
  secant update 复用 Jacobian，减少多轮 shooting 时的 `n_state` 次重复周期
  transient。每一步仍用真实一周期 transient 重新计算 residual，精度判据不变；
  可用 `jacobian_reuse=False` 恢复每轮完整有限差分，或设置
  `jacobian_rebuild_interval` 周期性重建。
- 返回一周期轨迹、`x0`、`x_end`、残差向量/范数、收敛标志和迭代历史。
  结果中还包含 `shooting_period_runs`、`shooting_jacobian_evals` 和
  `shooting_jacobian_reuses` 等性能计数器。
...
- `input_drive` 是小信号复幅值映射，例如差分输入为 `{"vip": 0.5, "vin": -0.5}`，
  单端输入为 `{"vin": 1.0}`。
- 默认启用两级性能优化：若 PSS 轨道和大信号输入均为常量，则直接使用 LTI
  `ac_solve`；否则将状态线性化和每个频点的输入 forcing 缓存在 `pss_result`
  内，重复 PAC/PNoise 调用可复用。结果中包含 `pac_period_runs`、
  `pac_state_cache_hit`、`pac_input_cache_hits` 等计数器。
...
- Numba 可用时，大规模 LPTV PNoise 会使用编译版 HB block 组装和
  ...
  计算 gm/gds，在小电流或 kink 附近自动回退原有限差分。
...
- 原生 `pmos_chopper_pac` 和 `pmos_chopper_pnoise`（第一性原理，无标定常数）已
  与 Spectre PSS/PAC/PNoise 对齐：D3 设计案例（f_chop=300 Hz，输出 RC 滤波，
  K=6 sidebands）的全分辨率 PNoise IRN 与 Cadence 偏差约 6% 以内。
