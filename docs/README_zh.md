...
设备上完成，无外部往返、无 license。JSON 电路格式、`python -m circuitopt` CLI、结构化的结果字典，都是刻意做成
...
| **[ngspice](https://ngspice.sourceforge.io/)** | 其内置 C-BSIM4 是 **FreePDK45** 的精确器件求值器 / oracle（`circuitopt/ngspice_char.py`），也负责解析 SKY130 的 binned 参数卡；由仓内 `tools/run-ngspice.sh` 包装脚本调用 | `NGSPICE_BIN` |
...
`.venv/pdk/` 和 `.venv/bin/openvaf-r`；BSIM4 源还可用 `BSIM4_VA` 单独覆盖。
...
`docs/module_overview_zh.md`）。没装的话，任何用到 `sky130.*` / `freepdk45.*` 模型类型
