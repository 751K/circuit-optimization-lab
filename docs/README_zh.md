...
- 默认：解析伴随谐波平衡（`method="pss_analytic_adjoint"`）。这是最通用路径，
  支持 bordered MNA 场景。
- 快速：time-domain Floquet PAC（`time_domain=True`，
  `method="pss_time_domain"`）。它先构建一次单周期 monodromy，再对每个频点求一个
  小的 quasi-periodic 边界系统，避免大型 `(2K+1)n` HB 矩阵；不支持的拓扑在
  `analytic=True` 时会自动回退到 HB。这条路径目前仍是 opt-in：2026-06-26
  三 corner 对入库 Cadence 参考的检查结果为 typical −0.44%、fast −0.27%，
  但 slow baseband gain 为 −1.89%，还不适合作为默认路径。
...
PNoise `maxsideband=10`、dec=10 噪声网格下，默认 HB PAC 增益和 PNoise IRN
误差均 <1%。time-domain PAC 是加速路径；在把它提升为新 chopper case 的默认路径前，
需要继续放在同一套 Cadence 回归里验证。
...
| Chopper PSS+PAC(HB)+PNoise（61 点，UI 尺寸）    | ~25.6 s |
| Chopper PSS+PAC(HB)+PNoise（121 点，UI 尺寸）   | ~48.9 s |
| Chopper PAC time-domain only（61 点，同一 PSS 轨道） | ~1.3 s |
| Chopper PAC time-domain only（121 点，同一 PSS 轨道） | ~1.9 s |
| 批量 sweep（200 候选, AC+noise）              | ~0.5 s  |

25.6s / 48.9s 这两行是默认通用 HB PAC 路径的全流程时间。对 rail-driven chopper，
打开 `time_domain=True` 后 PAC 不再是同量级瓶颈，但在 slow-corner 增益差异解决前，
它仍是 opt-in 加速路径。
...
rail-driven chopper 可用 `time_domain=True`（或 JSON `"time_domain": true`）进入
加速的 time-domain Floquet PAC，但目前应作为速度/诊断选项：当前 slow-corner
Cadence 检查 baseband 约 −1.9%。默认 HB 路径下，PAC 每频点求解仍是主要耗时：
61 点约 24–25s，121 点约 47–48s。后续 HB-only 优化应放在 factorization 复用或
批量线性求解上。
