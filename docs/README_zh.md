...
**当前版本：v1.1.0**（2026-07-13）—— 见
[CHANGELOG](https://github.com/751K/circuit-optimization-lab/blob/main/CHANGELOG.md)。

## 项目概述

一个**本地、对 Cadence 标定过的模拟电路仿真 + ML 驱动设计优化框架**——开放核心无需商业仿真器，
可选的 licensed foundry 模型保留在用户本机。目标是成为芯片设计
两个转变的**基础设施**：
...
（[`circuitopt/dataset.py`](https://github.com/751K/circuit-optimization-lab/blob/main/circuitopt/dataset.py)）→ ML 代理 + 筛选-校验优化器（[`circuitopt/surrogate.py`](https://github.com/751K/circuit-optimization-lab/blob/main/circuitopt/surrogate.py)、
...
设备上完成，无商业仿真器往返。JSON 电路格式、`python -m circuitopt` CLI、结构化的结果字典，都是刻意做成
机器可调用、机器可读的，好让 agent 从本地 shell 里驱动整个设计闭环。

求解器栈**对 Cadence Spectre 24.1 标定**——以 AT4000TG PMOS-OTFT 心电 AFE 为标定锚点（增益/带宽/IRN
通常 <1%）——并通过同一套引擎泛化到多个工艺：AT4000TG OTFT、**SKY130**（130nm，
OpenVAF/OSDI）、**FreePDK45**（45nm，ngspice-C），以及可选的本地 **TSMC28HPC+**
（28nm，内部 HSPICE 解析器 + 原生 Berkeley BSIM4.5）。
...
- **多工艺** — OTFT、SKY130、FreePDK45、TSMC28HPC+ 走同一条管线；TSMC 模型只保存在本机并由 Git 忽略，见 [TSMC28HPC+ 适配说明](tsmc28hpcp.md)。

求解器内部实现见 [核心求解器概览](module_overview_zh.md)。
...
`requirements-demo.txt` 补 Flask 给 Web demo；`serve` extra（`pip install -e ".[serve]"`）
补 FastAPI + uvicorn 给本地 HTTP 服务层（`circuit-opt serve`，见下文）。以上全部无需外部
仿真器、无需 license。

**外部工具与模型文件（仅硅 PDK 需要——不是 pip 依赖）。** 默认的 AT4000TG OTFT 工艺和所有分析没有这些也能
完整运行；只有 `sky130.*` / `freepdk45.*` / `tsmc28hpcp.*` 模型类型需要它们，缺失时会在第一次用到时报出清晰错误。

| 工具 | 在本项目中的作用 | 用哪个环境变量指向 |
|------|-----------------|------------------|
| **[OpenVAF-Reloaded](https://github.com/751K/OpenVAF-Reloaded)** —— [OpenVAF](https://github.com/pascalkuthe/OpenVAF) 的维护 fork | 把标准 Verilog-A 紧凑模型（BSIM4 等）编译成原生 `.osdi` 共享库；**SKY130** 器件路径通过 OSDI 宿主（`circuitopt/osdi_host.py`）加载它，由仓内 `tools/vacompile.sh` 包装脚本驱动 | `OPENVAF_BIN` / `OPENVAF_ROOT` |
| **[ngspice](https://ngspice.sourceforge.io/)** | **FreePDK45** 的 C-BSIM4 oracle，并负责解析 SKY130 参数卡；TSMC28HPC+ 仅在显式 oracle 对照时使用 | `NGSPICE_BIN` |
| PDK 模型文件 | SKY130/FreePDK45 走 `PDK_ROOT`；TSMC licensed 模型放项目内忽略目录或外部安装 | `PDK_ROOT`、`TSMC28_MODEL_DIR`、`TSMC28_PDK_ROOT` |

路径解析不依赖机器绝对路径：显式环境变量优先，其次当前 `VIRTUAL_ENV`，再尝试项目
`.venv`，可执行文件最后查 `PATH`。项目本地约定为 `.venv/ngspice/bin/ngspice`、
`.venv/pdk/`、`.venv/bin/openvaf-r`，以及 TSMC 的项目相对入口
`PDK/tsmc28hpcp/models/hspice/cln28hpcp_1d8_elk_v1d0_2p2.l`。TSMC 模型仍受用户的
foundry license/NDA 约束；这里只是不需要商业仿真器 license，也不会把模型提交到 Git。

### CLI 参考

`python -m circuitopt` 使用子命令（向后兼容——无子命令时默认为 `run`）：

```bash
# ── 分析调度（默认："run"） ──
python -m circuitopt examples/periodic_rc.json                          # 运行所有已配置的分析
python -m circuitopt examples/periodic_rc.json -a ac,noise,pss          # 指定分析类型
python -m circuitopt run examples/periodic_rc.json -a ac,noise          # 显式子命令

# ── 设计空间探索 ──
python -m circuitopt examples/afe_explore.json --explore -n 500         # --explore 标志（旧用法）
python -m circuitopt explore examples/afe_explore.json -n 500 --seed 1  # 子命令
...
# ── Chopper 分析 ──
python -m circuitopt chopper examples/afe_explore.json --level ideal    # 方波 LPTV
python -m circuitopt chopper examples/afe_explore.json --level pmos     # 静态相位 PMOS
python -m circuitopt chopper examples/afe_explore.json --level lptv     # PMOS 边带折叠
...
# ── FreePDK45 SAR ADC ──
python -m circuitopt adc examples/freepdk45_sar3.json --vin 0.7       # 单次闭环转换
python -m circuitopt adc examples/freepdk45_sar3.json --sweep 65      # DNL/INL ramp
python -m circuitopt adc examples/freepdk45_sar3.json --sine 32       # SNDR/SFDR/ENOB
...
| 概念               | 是什么                                  | 在哪定义                                             |
| ---------------- | ------------------------------------ | ------------------------------------------------ |
| **Topology（拓扑）** | 电路结构——有哪些节点、器件如何连接、输入输出在哪            | `circuitopt/topology.py`，或从 JSON 自动生成                  |
...
| **Solver（求解器）**  | 接收 拓扑 + 尺寸 + 偏置 → 输出结果（增益、噪声、波形…）的函数 | `circuitopt/ac_solver.py`、`circuitopt/transient_solver.py` 等 |
| **Device Model（器件模型）** | 抽象接口（`TransistorModel`）——求解器面向接口调用，不依赖具体模型；通过工厂切换模型类型 | `circuitopt/device_model.py`、`circuitopt/pmos_tft_model.py` |
...
`circuitopt.pac_solver.pac_solve` 和 `circuitopt.pnoise_solver.pnoise_solve`，输入为通用 `pss_solve`
...
`docs/module_overview_zh.md`。

### 7. 硅 PDK（SKY130 + FreePDK45 + TSMC28HPC+）
...
```bash
python -m circuitopt dataset examples/sky130_5t_ota.json -n 400 --out ds/ota          # 造一个硅数据集
python -m circuitopt.surrogate train ds/ota.npz --filter gain_dB:0:60 --out ds/ota.pkl # 只在工作区训练
python -m circuitopt.optimize examples/sky130_5t_ota.json ds/ota.pkl -n 50000 --top-k 10          # 筛选+校验（typical 工艺角）
python -m circuitopt.optimize examples/sky130_5t_ota.json ds/ota.pkl -n 50000 --corner ss          # 在慢角重新校验
```
...
工具链（见 `docs/module_overview_zh.md`）；未安装时求解器会给出清晰报错。**设计说明**：和任何单级
...
FreePDK45 瞬态会自动绕过小信号表征网格，生成完整四端 MOS 网表并调用 ngspice `.tran`，因此
BSIM4 的端口电荷、结电容和开关瞬态都由原生模型处理。`examples/freepdk45_sar3.json` 展示了
全差分 CDAC、CMOS 采样传输门、晶体管级比较器和 Python SAR 状态机的闭环：65 点 ramp 当前得到
`max|DNL|=0.108 LSB`、`max|INL|=0.062 LSB`、零 missing code；32 点相干正弦得到约 2.84-bit ENOB。

**TSMC28HPC+（28nm，0.9V core）** 使用 `"tsmc28hpcp.nmos"` / `.pmos`。适配器解析
foundry 的 `nch_mac` / `pch_mac`、HSPICE `.lib` 闭包和尺寸 bin，再由内部 Berkeley
BSIM4.5 后端完成 DC、AC、noise、transient、PSS、PAC、PNoise。正常仿真不启动 ngspice；
`tsmc28hpcp_ngspice.*` 仅为显式 oracle。单一必需模型文件、可迁移入口和 corner 说明见
[TSMC28HPC+ 适配说明](tsmc28hpcp.md)。licensed 模型目录已被 Git 忽略，禁止提交。
...
架在与 CLI 相同的求解器栈之上的本地 FastAPI HTTP 层——未来桌面 GUI 和 MCP server
共用的底座。同步端点（`health`/`capabilities`/`validate`/`solve`）加上 `explore`/`mc`
的后台任务端点（提交、轮询、WebSocket 进度、取消），全部是薄适配层，架在已有的单一
事实来源之上，CLI 和 HTTP 两个入口不会漂移。

```bash
pip install -e ".[serve]"
circuit-opt serve   # http://127.0.0.1:8341，swagger 在 /docs
```
...
FreePDK45 需 ngspice + FreePDK45 卡；TSMC28HPC+ 原生路径需 licensed 1d8 HSPICE 模型和
首次构建 BSIM4 后端时可用的 C 编译器，ngspice 仅用于可选 oracle。包装脚本在 `tools/`，
仓外只剩二进制。安装好之后把 `PDK_ROOT` 与二进制路径——`OPENVAF_BIN`（或 `OPENVAF_ROOT`,
解析为 `$OPENVAF_ROOT/target/release/openvaf-r`）与 `NGSPICE_BIN`——指过去（见
`docs/module_overview_zh.md`）。没装的话，任何用到 `sky130.*` / `freepdk45.*` / `tsmc28hpcp.*` 模型类型
...
| 文档                                       | 作用                     |
| ---------------------------------------- | ---------------------- |
| [核心求解器概览](module_overview_zh.md)           | 理解每个求解器的原理、导入依赖关系和标定数据 |
