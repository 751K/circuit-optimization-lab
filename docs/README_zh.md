...
（28nm，licensed HSPICE 模型经 ngspice）。
...
| **[ngspice](https://ngspice.sourceforge.io/)** | **FreePDK45** 的 C-BSIM4 oracle、**TSMC28HPC+** 全电路仿真器，并负责解析 SKY130 参数卡；由 `tools/run-ngspice.sh` 调用 | `NGSPICE_BIN` |
...
**TSMC28HPC+（28nm，0.9V core）** 使用 `"tsmc28hpcp.nmos"` / `.pmos`。适配器引用
foundry 的 `nch_mac` / `pch_mac` 子电路，展开 HSPICE `.lib` 依赖闭包，以 HSPICE 兼容模式运行
ngspice，并把层级工作点向量映射回 circuitopt 器件名。完整 `.tran`、`.ac`、`.noise`、`.op`
直接使用原模型 deck，本地优化循环可复用缓存表征网格。单一必需模型文件、可迁移入口和 corner
说明见 [TSMC28HPC+ 适配说明](tsmc28hpcp.md)。licensed 模型目录已被 Git 忽略，禁止提交。
...
FreePDK45 需 ngspice + FreePDK45 卡，TSMC28HPC+ 需 ngspice + licensed 1d8 HSPICE 模型。包装脚本在 `tools/`，
