...
本地 Python 电路求解器，用于模拟电路设计空间探索，已对 Cadence/Spectre 完成标定。首个应用场景是 **AT4000TG PMOS 薄膜晶体管 ECG AFE**（带 chopper 的心电模拟前端放大器）。第二个、行业标准的工艺——**硅 SKY130**（通过 OpenVAF 编译的 BSIM4）——接入了同一套求解器引擎和设计优化管线。
...
- **硅 PDK（SKY130）** — 真实 130nm CMOS 工艺（NMOS + PMOS），走的是和 OTFT 完全相同的
  DC/AC/noise/设计优化管线。
...
**`models`/硅器件报工具链错误。**
SKY130 PDK 需要一套外部工具链（OpenVAF + ngspice + SKY130 PDK 文件），它不是 pip
依赖——安装好之后把 `PDK_ROOT`/`OPENVAF_ROOT`/`NGSPICE_BIN` 指过去（见
`docs/futureplan.md` §9）。没装的话，任何用到 `sky130.*` 模型类型的电路会在第一次
用到时报出清晰错误；其余 PDK（默认的 AT4000TG OTFT）不受影响。依赖该工具链的测试
（`tests/test_sky130*.py`、`tests/test_osdi_host.py`）在工具链缺失时会干净地 skip。
