...
- 转写顺序（每步落 parity 测试再进下步）：
  ...
  ⑦ AC/noise 装配（`ac_mna` 对应）⑧ BSIM4 grid（调 co-bsim4，杀
  `numba_transient.py`）。
- Python 侧：`transient_solver` 增 rust 分派臂，marshal 塌缩；深路径 import
  与结果键逐字保持。
- 重写被钉住的测试：`test_transient_contracts` 改为引擎中立的接缝契约；
  `test_model_kernels`/`test_compiled_topology` 参数化 over engine。
