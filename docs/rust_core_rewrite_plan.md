...
> **文档状态：历史规划记录，方案已执行完毕。** 本文档记录的重写计划（R0–R7+）
> 已随 **v2.0.0**（2026-07-22 发布）全部落地：Rust 成为唯一计算引擎，numba/纯
> Python `_impl` 内核/运行时 cc 编译/OSDI/OpenVAF 均已移除，双发行版
> （`circuit-optimization` + `circuitopt-core`）精确 pin 同版本。当前权威状态见
> 根目录 `CHANGELOG.md` 的 `[2.0.0]` 条目；本文档保留作为设计决策与阶段验收的
> 历史记录，不再更新，文中的"当前""下一步""待定"等措辞对应规划撰写时点
> （2026-07-17/18），不代表现状。
>
