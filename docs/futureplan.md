...
- `bench_chopper` 频域路径：`ideal` 约 5 ms，`pmos_lptv` 约 22-25 ms。
- UI 锁定尺寸的 8 周期 finite-edge PMOS chopper transient：预热后约 0.57 s，
...
- PMOS quasi-LPTV / PAC-like sideband folding。✅（`pmos_chopper_lptv_analysis()`，
  用 PMOS 静态相位 response/noise 和有限边沿谐波权重做边带折叠，并对 UI 锁定
  `225 Hz`、switch `5000/30`、`rise/fall=20 us` 的 Spectre PSS/PAC/PNoise
  结果加入换向相位与周期噪声 PSD 标定：gain `21.370 dB` 对 `21.369 dB`，
  BW `738.6 Hz` 对 `721.9 Hz`，IRN `12.592 µV` 对 `12.591 µV`。）
...
- 自动化 PSS/PNoise 回归扩展，用于更多 switch 尺寸、频率、角落、charge injection
  周期噪声影响的批量校准。（待做）
...
一阶 transient stamp、PMOS quasi-LPTV/PAC-like folding，以及 hard-switched PMOS
transient 的 Spectre `tran` 对齐已完成；下一步是理想电压源 / 受控源，以及把
Spectre CSV/PSF 对比流程脚本化。
...
小规模稠密线性求解；当前稠密 Newton solve 已改为原地 `A*x = -R`，substep
输入插值也复用前一段结果，避免重复计算。后续可以继续推进：

- 把每个 interval/substep 的重试拆步和输出采样做成 compiled step plan，减少
  Python 调度开销；外层 time-loop 已试验过，当前未保留，因为实测没有收益。
...
2. ✅ 性能基准 `benchmarks/`——`bench_afe.py`（ac121/noise121/tran200）、`bench_model.py`（单管 7 操作 × 3 工作区）、`bench_chopper.py`（chopper 五项）、`bench_sweep.py`（N 候选 AC/AC+noise 吞吐量），均支持 cold/warm 和 Numba 对比。
...
6. ✅ Chopper——理想 LPTV 频域分析、PMOS 静态相位、有限边沿谐波、charge injection 一阶模型、PMOS quasi-LPTV/PAC-like folding、hard-switched PMOS transient 的 Spectre `tran` 对齐均已完成。
...
12. ⬜ 理想电压源 / 受控源。
13. 🔧 Spectre PSS/PNoise 批量回归扩展（更多 switch 尺寸、频率、角落）。
