...
统一编程接口、本地 FastAPI 服务层（`circuitopt/service/`，见 [本地服务 API](service_api_zh.md)）。
已完成的细节见 `docs/module_overview.md`、`README.md` 和 git 历史；本文档只记录**未来方向**。

---

## 核心架构判断

桌面客户端和 LLM 代理接口最终会共享同一个底座——**本地服务层**。这个底座（FastAPI +
WebSocket，`circuitopt/service/`）已经落地：`GET health`/`capabilities`、
`POST validate`/`solve` 四个同步端点，加上 `explore`/`mc` 的后台任务端点（提交/轮询/
WebSocket 进度/取消），均为薄适配层，直接调用 `run_analysis_suite`/`explore_from_dict`/
`mismatch_mc_from_dict` 等既有单一事实来源，不引入任何新的数值逻辑。`circuit-opt serve`
子命令 + `serve` 可选依赖组已可用；完整端点参考见 [本地服务 API](service_api_zh.md)。
策略是先把底座建好，再让桌面端和 LLM 端两个前端在其上并行生长，而不是各自造一套调用
逃生通道。
...
- **与 GUI/MCP 的衔接**：服务层已提供 `GET /api/v1/capabilities` 作为 GUI 下拉框的
  单一事实来源；桌面客户端（方向二）和 MCP server（方向三）具体怎么消费这套 API
  仍需在各自方向里落地。

---

## 方向二：macOS 桌面客户端（优先级高，服务层已就绪）
...
MCP server 与方向二共享同一个本地服务层（已就绪，见上文的核心架构判断），MCP 这一层
本身只是在已有 HTTP API 上再包一层薄适配。
...
服务层底座（原方向一）已经就绪。**方向三 → 方向二**：MCP 工程量小、且最贴合
"LLM 可本地调用"的项目定位，优先于工程量更大的桌面端；桌面端随后跟进。方向一的
剩余扩展（`optimize`/`dataset` 任务化、鉴权、结果缓存管理接口）按需伴随方向二/三
的实际需要推进，不单独抢排期。**方向四**是持续研究线，与工程方向并行推进，不占用
发布节奏。**方向五**按需启动，不预先排期。**方向六**伴随各方向的发布节奏推进，
不单独立项。
