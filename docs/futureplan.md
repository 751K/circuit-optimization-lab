...
6. **`branch_incidence` 扩展**：包含 VCVS+CCVS 支路（不再只是 vsources），PSS monodromy 和 PAC/PNoise HB 的 bordered 块现在对受控电压源正确。
...
- [x] **VCVS / CCCS / CCVS** ✅ **全分析路径补齐（2026-06-22）**
  ...
  - 2026-06-22 补齐：
    ...
    - `ac_mna.py`：`stamp_vcvs` / `stamp_cccs` / `stamp_ccvs` 原语
    - PSS/PAC/PNoise：`branch_incidence` 扩展为 vsource+VCVS+CCVS 并集
...
- ~~硅 PAC 扫频 / PNoise~~ ✅ **已完成（2026-07-04）**：`_assemble_pac_linearization_python`
  ...
  C=Ctt−Cti·X−Jti(W−V·X)，按偏置 memo；`ac_mna.stamp_dense_lti`），噪声采样直接走 ABC 的
...
| ~~模块拆分（`ac_solver`/`device_factory`/`compiled_topology`/CLI 单源化）~~ ✅ 已完成（2026-07-05） | 四阶段：(1) 新增 `device_factory.py`（器件构建 + corner 路由，leaf 模块）；(2) 新增 `dc_solver.py`（DC fallback + AFE 对称 seeding），`ac_solver.py` 瘦身为纯 AC 求解（515→355 行）；(3) 后端能力用 `TransistorModel.HAS_TERMINAL_LINEARIZATION`/`TRANSIENT_BACKEND` 类属性分派，取代 `isinstance(OsdiDevice)`，`index_array`/`term_arrays` 移入 `compiled_topology.py`，断开 `transient_solver`↔`osdi_transient` 双向 import；(4) `dataset.py`/`explore.py` 用 `add_cli_args`/`run_cli` 让子命令与独立模块入口共用同一套参数定义。见 `docs/core_overview.md` |
