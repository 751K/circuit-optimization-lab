...
| **Surrogate 基础** | 可信 teacher solver + JSON 设计空间 + Cadence 校准数据；**dataset builder v1 已落地**（`core/dataset.py`，provenance + 失败保留 + NPZ）；尚未实现 ML 训练/推理层 | 🟡 进行中 |
...
- [x] ~~**Dataset builder**~~ ✅ **v1 已完成（2026-07-02）** — `core/dataset.py` + `python -m core dataset` 子命令，
  ...
  **总是评估噪声**（每个收敛点带完整标签 `gain_dB/gain_peak_dB/bw_Hz/irn_uV/power_uW/area`），并写 manifest
  记录 schema 版本、solver commit(+dirty)、拓扑 hash、PDK、corner、变量范围、counts（供泛化边界判断）。
  产出 `<prefix>.jsonl`（可读）+ `.manifest.json`（provenance）+ `.npz`（稠密 X/Y + 掩码）+ 可选 `.parquet`。
  **schema 1.1** 引入 label 组：`ac_noise`（默认，6 指标）+ opt-in `transient`（`out_pp/out_mean/out_rms/slew_rate/final_value`，
  **激励无关**波形特征，复用配置已验证的 `periodic` transient，不假设阶跃语义）。`tests/test_dataset.py`
  守卫 schema/provenance/失败保留/确定性/label 组/Parquet/结构轴/CLI。**结构/激励设计轴**已支持：
  `<Cap>.C`（load）+ `periodic.frequency`（clock），逐候选重建电路（manifest 记 `kind`）。**label 组**含 `ac_noise` /
  `transient` / `pss`（`pss_converged` + residual/iters + 轨道 pp/mean）。**数据侧 v1 已完备**；下一步是 baseline
  指标 surrogate（引入首个 ML 依赖）。相位裕度（AC 环路）/ settling（阶跃）不属 PSS，另议。
...
- ~~**Surrogate dataset builder**~~ ✅ **v1 已完成（2026-07-02）** — `core/dataset.py` + `python -m core dataset`
  串起 explore 采样 → 已标定求解器评估 → 带 provenance（topology hash / corner / solver commit）、保留失败样本的
  `(design → metrics)` 数据集（JSONL + manifest + NPZ）。下一步是 **baseline 指标 surrogate**（MLP / GBT / GP，
  先覆盖 AC/noise/power/area，输出均值 + 误差估计），以及 Parquet 导出、波形标签、clock/load 参数轴。
