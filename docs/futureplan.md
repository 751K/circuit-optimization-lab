...
calibration/                ~450 行  (4 个 case 目录, 含 PSFASCII 参考文件)
```
...
2. **扩展验证覆盖** 🟡 — SC 低通本地调试 ✅（2026-06-22 三个 bug 修复），待 Cadence 对标
...
core/calibration.py         load_reference → run_local → compare_* → format_report，逐指标 + per-case 容差
        ↓
tests/test_calibration.py   回归守卫：4 个 case 全部 PASS 才算通过
```
...
- [ ] **替换经验常数** — `_CADENCE_PMOS_CHOPPER_CONVERSION_PHASE_RAD`=24.93° 和 `_PERIODIC_NOISE_PSD_SCALE`=1.0355 用物理修正替代。闭环已是守住容差的回归保证，此项不急
...
- [ ] **编写 SC 低通的 Cadence 对标** — 在 Spectre 跑相同 SC 低通的 pss/pac/pnoise，导出参考数据到 `calibration/`（ROI 最高）
- [ ] **扩展对标覆盖** — 更多 switch 尺寸、更多 f_chop、三 corner × 多频率组合
- [ ] 评估 gear2 vs BE 对 PSS 轨道精度的系统性影响（JSON dispatch 默认 gear2，PAC 结果略差于脚本 BE 路径）
- [ ] 给 dispatch 的 `_PSS_KWARGS` 加 `integration_method`，让 JSON 可指定积分方法
...
**第 1/3/4 步已于 2026-06-21 全部完成。** 第 2 步（扩展验证覆盖）SC 低通已于 2026-06-22 完成 bug
修复 —— PSS residual 10× 改善 (0.007V)，PAC BW Δ=+8%，PNoise IRN 从 flicker 过估计中修正
(20.6→4.46 µVrms)。本地结果现在已经可靠。

当前最有价值的投入：

- **SC 低通的 Cadence 对标** — 用 `core/cadence_netlist.py` 生成网表，跑 Spectre PSS/PAC/PNoise，
  将参考数据加入 `calibration/`，让周期分析验证形成完整闭环。这是第 2 步剩余工作中 ROI 最高的。
- **PNoise numba kernel 更新** — `pnoise_fold_psd_numba` 的签名和逻辑已更新为 √PWR 调制幅值向量，
  需确认 numba 编译缓存已清除（`__pycache__` 无过期 `.nbc` 文件）。
- **dispatch 完善** — 加 `integration_method` 到 `_PSS_KWARGS`，让 JSON 可指定积分方法。
