...
| 低频增益 > 40 dB | **57.7 dB**(通带,PVT 全 27 角 ≥41.7 dB) | ✅ |
| UGBW ≥ 0.1 GHz | **109 MHz**(恒 gm 偏置下 PVT 全角 109–110 MHz) | ✅ |
| 功耗 | **14.0 µW @ 1.0 V**(见 §4;100 MHz 处地板 12.9 µW) | — |
...
偏置轨(1.0 V):VCMI = 0.65(输入对 Vgs)、VCM_REF = 0.50(≈中轨,最优输出 CM)、VB_N = 0.429
(尾电流)、VB_CN = 0.814(N 级联栅,把输入对 Vds 顶到 ~0.13 V 饱和)、VB_CP = 0.35(P 级联栅)。
输入对 L = 0.15 µm(比其余核心管 L = 0.1 长——见 §5 的 PVT 增益裕量教训);负载/尾 L = 0.2。
...
**最低功耗 = 14.0 µW**(gain 57.7 dB / UGBW 109 MHz / PM 87° / OCM 526 mV @ 1.0 V)。
...
UGBW 线性,每加 100 MHz 约 +16 µW。选定的 14.0 µW/109 MHz 点正落在这条前沿上;若只求达标,
100 MHz 处地板 **12.9 µW**。

**功耗地板讨论**:UGBW 约束下主体电流下限 = 2·gm1/(gm/Id)max ≈ 2·126µS/23 ≈ 11 µA(11 µW);
CMFB + 镜像支路再加 ~3 µA。14 µW 距理想地板 ~25%,再压空间在:CL 减半(规格允许则主体电流直接
减半)、CMFB 缩(以 CM 精度/环速换)、或接受 ss 角 UGBW 裕量更薄。**结论:CL = 0.2 pF、全 PVT
达标前提下,~13–14 µW 是这个架构在 1.0 V 的实际下限。**
...
| 方案 | 增益范围 | UGBW 范围 | 功耗范围 |
|---|---|---|---|
| 固定 VB_N(电压偏置) | 41.0–61.1 dB | **44–159 MHz** ❌ | 5.9–23.2 µW |
| 恒 gm 偏置(逐点解 VB_N 使 gm1 = 140 µS) | **41.7–59.6 dB** ✅ | **109–110 MHz** ✅ | 12.6–19.1 µW |

- 固定电压偏置在高温/慢角**尾电流塌缩**:90 °C 迁移率降 → Id 掉 ~30%,UGBW 从 109 掉到
  67 MHz(nom)、**44 MHz(ss)——远不达标**(增益因输入对 L=0.15 的裕量仍保住 ≥41 dB)。
- **恒 gm 偏置**(片上即经典恒 gm 电流基准做的事,本报告用逐点二分 VB_N 仿真其行为)把
  UGBW 钉在 109–110 MHz,增益全角 ≥41.7 dB(最差 ff/0.9 V/90 °C)。恒 gm 下功耗随温度上升
  (90 °C 比 30 °C 贵 ~20%:gm ∝ I/T,保 gm 必须加流)——这是物理,不是缺陷。
