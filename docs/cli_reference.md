...
| 命令 | 用途 |
|------|------|
| `python -m circuitopt <circuit.json>` | 按 JSON 配置跑分析（AC/Noise/Tran/PSS/PAC/PNoise） |
| `python -m circuitopt run <circuit.json> -a ac,noise` | 显式指定分析子集 |
| `python -m circuitopt explore <circuit.json> -n 300` | 设计空间探索（LHS/随机采样 → Pareto） |
...
| `python -m circuitopt.optimize <circuit.json> m.pkl -n 100000 --top-k 20` | 筛选 → Pareto → solver 校验闭环 |
| `python -m circuitopt.surrogate_torch optimize <circuit.json> m.pt --verify` | 可微 surrogate 梯度设计优化（torch/MPS） |
...
# 只跑 AC 和 Noise
python -m circuitopt examples/afe_explore.json -a ac,noise

# 显式子命令形式（等价）
python -m circuitopt run examples/afe_explore.json -a ac,noise
```
...
> `dc_guesses`）——硅配置（含 FreePDK45）经 `python -m circuitopt run` / `python -m circuitopt explore` 直接可用。
...
```bash
python -m circuitopt explore examples/afe_explore.json -n 300 --seed 42
python -m circuitopt explore examples/afe_explore.json -n 200 --method random
```
...
| `--no-numba` | flag | — | 禁用 Numba（仅 `python -m circuitopt explore` 子命令有此参数；独立入口 `python -m circuitopt.explore` 没有） |
| `--quiet` | flag | — | 不打印逐候选进度 |

> `explore` 的 CLI 参数由 `circuitopt/explore.py` 的 `add_cli_args(parser)` / `run_cli(args)` 定义，
> `python -m circuitopt explore` 子命令和独立入口 `python -m circuitopt.explore` 共用同一份定义，因此参数
...
python -m circuitopt corners examples/afe_explore.json --freqs-num 61 --freqs-stop 5000
```
...
python -m circuitopt mc examples/afe_explore.json --corner slow --quiet
```
...
```bash
# 理想方波 chopper
python -m circuitopt chopper examples/afe_explore.json --level ideal --f-chop 225

# 硬开关瞬态
python -m circuitopt chopper examples/afe_explore.json --level transient --f-chop 225 --n-periods 8

# PSS+PAC+PNoise（第一性原理）
python -m circuitopt chopper examples/afe_explore.json --level pnoise --f-chop 225
```
...
```bash
python -m circuitopt plot                      # 全部 4 张：AFE 瞬态 + chopper 轨道 + AC + PAC Bode
python -m circuitopt plot transient            # 只瞬态两张（afe + chopper）
python -m circuitopt plot bode                 # 只 Bode 两张（ac + pac）
python -m circuitopt plot afe --f0 20 --amp 1e-3        # 单张：AFE 正弦放大
python -m circuitopt plot chopper --f-chop 200 --input-diff 2e-3
python -m circuitopt plot pac --npts 121 --out-dir /tmp/plots
```
...
是 ML surrogate 路线（架构见 `docs/module_overview.md`，未来方向见 `docs/futureplan.md`）的数据生成前置。

```bash
python -m circuitopt dataset examples/single_stage.json -n 500 --out ds/run1
python -m circuitopt dataset examples/afe_explore.json -n 1000 --corner slow --seed 7 --out ds/slow
python -m circuitopt dataset examples/single_stage.json -n 200 --no-npz --quiet
```
...
| `--no-numba` | flag | — | 禁用 Numba（仅 `python -m circuitopt dataset` 子命令有此参数；独立入口 `python -m circuitopt.dataset` 没有） |
| `--quiet` | flag | — | 不打印进度 |

> `dataset` 的 `--corner` 帮助文案与 `run`/`explore` 一致：OTFT `typical/slow/fast`，
> 或硅 `tt/ss/ff/sf/fs`（SKY130）/ `nom/ss/ff`（FreePDK45）。
>
> `dataset` 的 CLI 参数由 `circuitopt/dataset.py` 的 `add_cli_args(parser)` / `run_cli(args)` 定义，
> `python -m circuitopt dataset` 子命令和独立入口 `python -m circuitopt.dataset` 共用同一份定义，因此参数表
...
```bash
python -m circuitopt dataset examples/sky130_chopper.json --labels pss,pac,pnoise -n 200 --out chopper_ds
```
...
```bash
python -m circuitopt dataset examples/sky130_5t_ota.json -n 400 --out ds/ota          # 造硅数据集
python -m circuitopt.surrogate train ds/ota.npz --filter gain_dB:0:60 --out ota.pkl   # 训练（--filter 只学工作区，剔除甩轨角）
python -m circuitopt.optimize examples/sky130_5t_ota.json ota.pkl -n 50000 --top-k 10 # 筛选→Pareto→solver 校验
...
## 2. 校准 CLI：`python -m circuitopt.calibration`

Cadence/Spectre 校准闭环检查。入口文件 `circuitopt/calibration.py`。
...
# 单个 case
python -m circuitopt.calibration calibration/amp_design3_typical/

# 只跑指定分析
python -m circuitopt.calibration calibration/chopper_design3_typical/ --analyses pac,pnoise

# CI 友好（JSON 输出 + 非零退出码 = 失败）
python -m circuitopt.calibration --all --json

# 放宽 3 倍容差（调试用）
python -m circuitopt.calibration --all --relaxed
```
...
```bash
python -m circuitopt.ac_solver          # 硬编码尺寸跑一次 AC
python -m circuitopt.noise_solver       # 硬编码尺寸跑一次 Noise
python -m circuitopt.transient_solver   # 硬编码尺寸跑一次 Transient
python -m circuitopt.pmos_tft_model     # 打印单管 Id / Cgss / Cgdd / 噪声 PSD
```
...
> **生效机制与限制**：`numba_kernels` 的 `USE_NUMBA` 标志在该模块首次 import 时烙死为常量，事后设环境变量是静默 no-op。`--no-numba` 之所以能生效，是因为 `circuitopt/__init__.py` 在其求解器 import（会连带传递 import `numba_kernels`）之前，先对 `sys.argv` 预扫该 flag 并设好 `CIRCUIT_USE_NUMBA=0`；在 `python -m circuitopt …` 下 `__init__` 先于 `__main__.py` 执行，所以命令行用法（如上）总能生效。但如果从 Python 内部直接调用 `circuitopt.__main__.main()`（跳过了 `python -m circuitopt` 的正常 import 顺序，或在那之前已 import 了某个求解器模块），预扫可能被绕过——这种情况下 `_assert_numba_flag()` 守卫会检测到 `--no-numba` 请求但 Numba 仍激活，直接抛 `SystemExit` 响亮报错，而不是静默地假装关闭了 Numba。

### 退出码

全部返回 0 表示成功，非零表示失败。`python -m circuitopt.calibration --all` 任一 case 失败则退出码为 1，适合 CI 集成。
...
- name: calibration regression
  run: python -m circuitopt.calibration --all --json

- name: circuitopt analysis smoke test
  run: |
    python -m circuitopt run examples/periodic_rc.json -a ac,noise,pss
    python -m circuitopt corners examples/afe_explore.json --quiet

- name: mismatch MC (spot check)
  run: python -m circuitopt mc examples/afe_explore.json -n 50 --seed 1 --quiet
```
...
python -m circuitopt.calibration calibration/amp_design3_typical/ && \
python -m circuitopt run examples/periodic_rc.json -a pss --quiet
```

---

## 10. ML Surrogate + 优化闭环：`python -m circuitopt.surrogate` / `circuitopt.optimize`
...
```bash
python -m circuitopt.optimize examples/afe_explore.json results/models/afe_typical.pkl -n 100000 --top-k 20
```
...
python -m circuitopt.surrogate_torch train results/datasets/afe/afe_typical_train.npz \
...
python -m circuitopt.surrogate_torch optimize examples/afe_explore.json results/models/afe_torch.pt \
    --steps 600 --verify
```
