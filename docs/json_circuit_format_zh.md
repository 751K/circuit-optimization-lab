...
- 电阻、电容、理想直流电流源、VCCS（压控电流源）、理想电压源（真·MNA）。
- DC/AC/noise/transient 共享拓扑（电阻含热噪声；VCCS 与理想电压源为理想/无噪声）。
- 单端或差分输出。
- 固定 load capacitance。
...
- NMOS 或其他紧凑模型实现（接口已就绪，模型实现待补充）。
- 其他受控源（CCCS、CCVS、VCVS）、开关 / 时变元件。
- 多输出同时分析。
- 层次化子电路。
- SPICE 语法解析。
- JSON 中按器件声明 `"model"` 字段（当前默认 ``"pmos_tft"``）。

电路描述能力的扩展通过 ``core/device_model.py``（器件模型注册表）和 ``core/ac_mna.py``（MNA 电压源 / VCCS stamp 原语）实现。
