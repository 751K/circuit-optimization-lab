...
[English](json_circuit_format.md) | [中文说明](README_zh.md) | [核心求解器概览](core_overview_zh.md)
...
`core.pnoise_solver.pnoise_solve(..., time_domain=True)`。

### `explore`

可选。设计空间探索配置——待扫描的变量及范围、可行性约束（gain/BW/IRN/power/area）、
优化目标。被 `core.explore`（采样→评估→约束→Pareto 选择）、`core.dataset`
（采样→评估每个候选、不过滤——产出带标签的训练集）、`core.optimize`（用训练好的
...
`targets` 还支持结构化设计轴（每个都会逐候选重建电路；由 `core.dataset`/
`core.optimize` 消费，`core.explore` 不支持）：
...
- DC 初值。
- 器件模型抽象（``core/device_model.py``）——支持新增模型类型而不改求解器代码。
...
电路描述能力的扩展通过 ``core/device_model.py``（器件模型注册表）和 ``core/ac_mna.py``（MNA 电压源 / VCCS / VCVS / CCCS / CCVS stamp 原语）实现。
