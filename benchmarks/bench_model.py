...
operation.  Numba is enabled by default when installed, so cold runs include
any first-call JIT work unless CIRCUIT_USE_NUMBA=0/false/off disables it.

Usage:
  python3 -m benchmarks.bench_model --warm-runs 3
  CIRCUIT_USE_NUMBA=0 python3 -m benchmarks.bench_model --warm-runs 3 --json
"""
import argparse
import json
import os
...
def bench_case(name, fn, summarize, warm_runs):
    ...
    for _ in range(warm_runs):
        warm_out, dt = _time_call(lambda: fn(dev))
        warm_ms.append(dt)
...
def main():
    ...
    report = run_benchmarks(args.warm_runs)
    if args.json:
