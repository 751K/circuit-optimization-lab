...
def _osdi_newton_step_impl(R, J, T, gmin):
    """One OSDI internal-node Newton step (osdi_host.operating_point inner loop).

    Composes what would otherwise be 4-5 numpy calls on tiny arrays (abs/max/
    isfinite/solve/damp) into one kernel call. Returns ``(rmax, finite, ok, dv)``:
    ``rmax`` = max|R[T:]| (internal residual norm), ``finite`` = internal
    residuals all finite, ``ok`` = damped GEPP solve of
    ``(J[T:,T:] + gmin*I) dv = -R[T:]`` succeeded (``dv`` valid only then).
    """
    n = R.shape[0]
    ni = n - T
    rmax = 0.0
    ...
    for i in range(ni):
        ...
        A[i, i] += gmin
        dv[i] = R[T + i]
    ok, dv = _solve_dense_neg_rhs_inplace_impl(A, dv)
    if not ok:
        return rmax, True, False, dv
    smax = 0.0
    for i in range(ni):
        a = abs(dv[i])
        if not math.isfinite(a):
        ...
        if a > smax:
            smax = a
    if smax > 2.0:                       # damp large Newton steps
        f = 2.0 / smax
        for i in range(ni):
            dv[i] *= f
    return rmax, True, True, dv


def _osdi_schur_impl(J, T, gmin):
    """Schur complement of the reduced resistive Jacobian onto the terminals:
    ``G = Jtt - Jti (Jii + gmin*I)^-1 Jit`` via one GEPP factorization of the
    internal block applied to the T right-hand-side columns.

    Returns ``(ok, G)``; on ``ok=False`` (singular/non-finite pivot) the caller
    ...
    if ni <= 0:
        ...
        return True, G
    A = np.empty((ni, ni))
    X = np.empty((ni, T))            # becomes (Jii + gmin*I)^-1 Jit
    for i in range(ni):
        for j in range(ni):
            A[i, j] = J[T + i, T + j]
        A[i, i] += gmin
...
def _osdi_op_solve_impl(f_ev, f_lr, f_lj, handle, inst, model, info, resid_ptr,
                        v, resid, jac_vals, node_sel, node_red_idx,
                        jac_sel, jac_flat, redv, T, iters, tol, gmin):
    """Full OSDI internal-node Newton for one start, OSDI eval called in-loop.

    The hot half of ``osdi_host.Device.operating_point``: per iteration it
    scatters ``redv`` into the device node vector ``v``, calls the OSDI eval /
    load fn pointers (``f_ev``/``f_lr``/``f_lj`` — ctypes ``CFUNCTYPE`` objects
    with void*-only signatures, passed as *runtime arguments* so ``cache=True``
    never bakes a process-specific address), assembles the reduced (R, J), and
    steps via :func:`_osdi_newton_step_impl` — with the same polish step,
    two-tier unphysical-bias early exit, and singular-block least-squares
    rescue as the host. ``redv`` is updated in place.

    The per-eval body must mirror ``osdi_host.Device._eval_reduced`` (kept as
    the readable single-eval reference).

    Returns ``(status, R, J)`` with R/J from the last eval — status 0 =
    converged, 1 = budget exhausted or unphysical-bias bail, 2 = non-finite
    divergence (caller tries the next start), 3 = OSDI $fatal (caller raises).
    """
    n_red = redv.shape[0]
    ni = n_red - T
    nn = v.shape[0]
    ...
    for it in range(iters):
        for i in range(nn):
            v[i] = 0.0
        for k in range(node_sel.shape[0]):
            v[node_sel[k]] = redv[node_red_idx[k]]
        ret = f_ev(handle, inst, model, info)
        if ret & 2:                               # EVAL_RET_FLAG_FATAL
            return 3, R, J
        # load_* functions STAMP (+=), SPICE-style: clear destinations first
        for i in range(nn):
            resid[i] = 0.0
        for i in range(jac_vals.shape[0]):
            jac_vals[i] = 0.0
        f_lr(inst, model, resid_ptr)
        f_lj(inst, model)
        for i in range(n_red):
            ...
            for j in range(n_red):
                J[i, j] = 0.0
        for k in range(node_sel.shape[0]):
            R[node_red_idx[k]] += resid[node_sel[k]]
        for k in range(jac_sel.shape[0]):
            flat = jac_flat[k]
            J[flat // n_red, flat % n_red] += jac_vals[jac_sel[k]]
        if ni == 0:
            return 0, R, J
        rmax, finite, ok, dv = _osdi_newton_step_impl(R, J, T, gmin)
        if not finite:
            return 2, R, J                        # diverged → next start
        if polish:
            return 0, R, J
        polish = rmax < tol                       # one corrector step, then stop
        if (it >= 12 and rmax > 1e2) or (it >= 30 and rmax > 1e-1):
            # ampere-scale internal residual this deep into a damped march:
            # the bias itself is unphysical — don't burn the full budget
            return 1, R, J
        if not ok:
            # singular internal block → least-squares rescue step
            A = np.empty((ni, ni))
            b = np.empty(ni)
            for i in range(ni):
                for j in range(ni):
...
def _osdi_dev_eval_impl(f_ev0, f_lr0, f_lj0, f_lrr0, f_ljr0, handle0,
                        f_ev1, f_lr1, f_lj1, f_lrr1, f_ljr1, handle1,
                        lib_a, i,
                        ...
                        nred_a, int_base_a, T, X, input_now, redval):
    """Evaluate OSDI device ``i`` at the circuit state ``X``: build its node
    voltages, run one combined resistive+reactive eval, load R/Q/Jr/Jq into
    the bound 2D rows. Returns True unless the model raised $fatal.

    Two compiled-model libraries are supported (``lib_a[i]`` selects the
    fn-pointer set) — mixed-model circuits, e.g. BSIM4 + BSIM3 devices in one
    testbench. Single-lib callers pass the same pointers for both sets.
    """
    nred = nred_a[i]
    for r in range(nred):
        if r < T:
            redval[r] = _term_value_impl(term_kind2d[i, r], term_ref2d[i, r],
                                         term_val2d[i, r], X, input_now)
        else:
            redval[r] = X[int_base_a[i] + (r - T)]
    n_nodes = nmap2d.shape[1]
    for j in range(n_nodes):
        m = nmap2d[i, j]
        v2d[i, j] = redval[m] if m >= 0 else 0.0
    # int() casts are no-ops under numba; in the interpreted (numba-off) path
    # they turn np.int64 into the plain ints ctypes fn pointers require
    inst = int(inst_a[i])
    model = int(model_a[i])
    info = int(info_a[i])
    if lib_a[i] == 0:
        ret = f_ev0(handle0, inst, model, info)
    else:
        ret = f_ev1(handle1, inst, model, info)
    if ret & 2:                                   # EVAL_RET_FLAG_FATAL
        return False
    for j in range(n_nodes):
        residR2d[i, j] = 0.0
    ...
    for k in range(n_jac):
        ...
        react2d[i, k] = 0.0
    if lib_a[i] == 0:
        f_lr0(inst, model, int(residR_ptr_a[i]))
        f_lrr0(inst, model, int(residQ_ptr_a[i]))
        f_lj0(inst, model)
        f_ljr0(inst, model, 1.0)
    else:
        f_lr1(inst, model, int(residR_ptr_a[i]))
        f_lrr1(inst, model, int(residQ_ptr_a[i]))
        f_lj1(inst, model)
        f_ljr1(inst, model, 1.0)
    return True


def _osdi_stamp_elements_impl(
    ...
    """Stamp every non-transistor element into (F, Aflat) at circuit state X.

    Shared by the fixed-grid and adaptive OSDI transient kernels (single
    source). ``a0/a1/a2`` are the dQ/dt discretization coefficients applied to
    the linear capacitors' branch voltages against their histories vab1/vab2.
    Convention: F = sum of currents LEAVING each KCL row (+ gmin*V), the
    negation of ``CompiledTopology.dc_residuals``.
    """
    # resistors
    for e in range(res_g.shape[0]):
        va = _term_value_impl(res_a_kind[e], res_a_ref[e],
                              res_a_val[e], X, input_now)
        vb = _term_value_impl(res_b_kind[e], res_b_ref[e],
                              res_b_val[e], X, input_now)
        cur = res_g[e] * (va - vb)
        ai = res_ai[e]
        bi = res_bi[e]
        if ai >= 0:
            F[ai] += cur
            Aflat[ai * ntot + ai] += res_g[e]
            if bi >= 0:
                Aflat[ai * ntot + bi] -= res_g[e]
        if bi >= 0:
            F[bi] -= cur
            Aflat[bi * ntot + bi] += res_g[e]
            if ai >= 0:
                Aflat[bi * ntot + ai] -= res_g[e]
    # linear capacitors: same dQ/dt discretization on the branch voltage
    for e in range(cap_value.shape[0]):
        va = _term_value_impl(cap_a_kind[e], cap_a_ref[e],
                              cap_a_val[e], X, input_now)
        vb = _term_value_impl(cap_b_kind[e], cap_b_ref[e],
                              cap_b_val[e], X, input_now)
        vab = va - vb
        icap = cap_value[e] * (a0 * vab + a1 * vab1[e] + a2 * vab2[e])
        geq = cap_value[e] * a0
        ai = cap_ai[e]
        bi = cap_bi[e]
        if ai >= 0:
            F[ai] += icap
            Aflat[ai * ntot + ai] += geq
            if bi >= 0:
                Aflat[ai * ntot + bi] -= geq
        if bi >= 0:
            F[bi] -= icap
            Aflat[bi * ntot + bi] += geq
            if ai >= 0:
                Aflat[bi * ntot + ai] -= geq
    # ideal current sources (value flows p -> q)
    for e in range(isrc_value.shape[0]):
        if isrc_pi[e] >= 0:
            F[isrc_pi[e]] += isrc_value[e]
        if isrc_qi[e] >= 0:
            F[isrc_qi[e]] -= isrc_value[e]
    for e in range(dyn_idx.shape[0]):
        cur = input_now[dyn_idx[e]]
        if dyn_pi[e] >= 0:
            F[dyn_pi[e]] += cur
        if dyn_qi[e] >= 0:
            F[dyn_qi[e]] -= cur
    # ideal voltage sources: branch current unknown + constraint row
    for e in range(vs_bi.shape[0]):
        bi = vs_bi[e]
        ib = X[bi]
        E = vs_e_const[e] if vs_e_idx[e] < 0 else input_now[vs_e_idx[e]]
        vp = 0.0
        vq = 0.0
        if vs_pi[e] >= 0:
            vp = X[vs_pi[e]]
            F[vs_pi[e]] += ib
            Aflat[vs_pi[e] * ntot + bi] += 1.0
        if vs_qi[e] >= 0:
            vq = X[vs_qi[e]]
            F[vs_qi[e]] -= ib
            Aflat[vs_qi[e] * ntot + bi] -= 1.0
        F[bi] += vp - vq - E
        if vs_pi[e] >= 0:
            Aflat[bi * ntot + vs_pi[e]] += 1.0
        if vs_qi[e] >= 0:
            Aflat[bi * ntot + vs_qi[e]] -= 1.0
    # VCCS: i = gm*(V_cp - V_cn) enters p (matches dc_residuals; this
    # kernel's F is the NEGATED convention -> F[p] -= i)
    for e in range(vccs_gm.shape[0]):
        vcp = _term_value_impl(vccs_cp_kind[e], vccs_cp_ref[e],
                               vccs_cp_val[e], X, input_now)
        vcn = _term_value_impl(vccs_cn_kind[e], vccs_cn_ref[e],
                               vccs_cn_val[e], X, input_now)
        cur = vccs_gm[e] * (vcp - vcn)
        pi = vccs_pi[e]
        qi = vccs_qi[e]
        if pi >= 0:
            F[pi] -= cur
            if vccs_cp_kind[e] == 0:
                Aflat[pi * ntot + vccs_cp_ref[e]] -= vccs_gm[e]
            if vccs_cn_kind[e] == 0:
                Aflat[pi * ntot + vccs_cn_ref[e]] += vccs_gm[e]
        if qi >= 0:
            F[qi] += cur
            if vccs_cp_kind[e] == 0:
                Aflat[qi * ntot + vccs_cp_ref[e]] += vccs_gm[e]
            if vccs_cn_kind[e] == 0:
                Aflat[qi * ntot + vccs_cn_ref[e]] -= vccs_gm[e]
    # VCVS: V_p - V_q = mu*(V_cp - V_cn); branch current unknown at bi
    for e in range(vcvs_mu.shape[0]):
        bi = vcvs_bi[e]
        ib = X[bi]
        if vcvs_pi[e] >= 0:
            F[vcvs_pi[e]] += ib
            Aflat[vcvs_pi[e] * ntot + bi] += 1.0
        if vcvs_qi[e] >= 0:
            F[vcvs_qi[e]] -= ib
            Aflat[vcvs_qi[e] * ntot + bi] -= 1.0
        vp = _term_value_impl(vcvs_p_kind[e], vcvs_p_ref[e],
                              vcvs_p_val[e], X, input_now)
        vq = _term_value_impl(vcvs_q_kind[e], vcvs_q_ref[e],
                              vcvs_q_val[e], X, input_now)
        vcp = _term_value_impl(vcvs_cp_kind[e], vcvs_cp_ref[e],
                               vcvs_cp_val[e], X, input_now)
        vcn = _term_value_impl(vcvs_cn_kind[e], vcvs_cn_ref[e],
                               vcvs_cn_val[e], X, input_now)
        F[bi] += vp - vq - vcvs_mu[e] * (vcp - vcn)
        if vcvs_p_kind[e] == 0:
            Aflat[bi * ntot + vcvs_p_ref[e]] += 1.0
        if vcvs_q_kind[e] == 0:
            Aflat[bi * ntot + vcvs_q_ref[e]] -= 1.0
        if vcvs_cp_kind[e] == 0:
            Aflat[bi * ntot + vcvs_cp_ref[e]] -= vcvs_mu[e]
        if vcvs_cn_kind[e] == 0:
            Aflat[bi * ntot + vcvs_cn_ref[e]] += vcvs_mu[e]
    # CCCS: I_out = beta * I_ctrl enters p (I_ctrl = a vsource branch)
    for e in range(cccs_beta.shape[0]):
        cur = cccs_beta[e] * X[cccs_ctrl_bi[e]]
        if cccs_pi[e] >= 0:
            F[cccs_pi[e]] -= cur
            Aflat[cccs_pi[e] * ntot + cccs_ctrl_bi[e]] -= cccs_beta[e]
        if cccs_qi[e] >= 0:
            F[cccs_qi[e]] += cur
            Aflat[cccs_qi[e] * ntot + cccs_ctrl_bi[e]] += cccs_beta[e]
    # CCVS: V_p - V_q = gamma * I_ctrl; branch current unknown at bi
    for e in range(ccvs_gamma.shape[0]):
        bi = ccvs_bi[e]
        ib = X[bi]
        if ccvs_pi[e] >= 0:
            F[ccvs_pi[e]] += ib
            Aflat[ccvs_pi[e] * ntot + bi] += 1.0
        if ccvs_qi[e] >= 0:
            F[ccvs_qi[e]] -= ib
            Aflat[ccvs_qi[e] * ntot + bi] -= 1.0
        vp = _term_value_impl(ccvs_p_kind[e], ccvs_p_ref[e],
                              ccvs_p_val[e], X, input_now)
        vq = _term_value_impl(ccvs_q_kind[e], ccvs_q_ref[e],
                              ccvs_q_val[e], X, input_now)
        F[bi] += vp - vq - ccvs_gamma[e] * X[ccvs_ctrl_bi[e]]
        if ccvs_p_kind[e] == 0:
            Aflat[bi * ntot + ccvs_p_ref[e]] += 1.0
        if ccvs_q_kind[e] == 0:
            Aflat[bi * ntot + ccvs_q_ref[e]] -= 1.0
        Aflat[bi * ntot + ccvs_ctrl_bi[e]] -= ccvs_gamma[e]
    # gmin on KCL rows (external nodes + device internals; NOT the
    # constraint rows n_kcl..n_aug-1)
    for r in range(ntot):
        if r < n_kcl or r >= n_aug:
            F[r] += gmin * X[r]
            Aflat[r * ntot + r] += gmin
...
def _osdi_transient_grid_impl(
        ...
        ccvs_ctrl_bi, ccvs_gamma,
        n_kcl, n_aug, T,
        newton_maxit, newton_vtol, step_limit, gmin):
    """Fixed-grid transient for OSDI-device circuits (silicon Phase B).

    Global-fold formulation: the unknown vector ``X`` is [external solved nodes
    + ideal-vsource branch currents] (= n_aug) ++ every device's internal nodes
    (device i's internals at ``int_base_a[i] ..``) — one circuit-level Newton
    per step, no nested per-device solve and no quasi-static approximation
    (internal-node charge dynamics included). Per device and Newton iteration a
    single OSDI eval (resistive+reactive residual+Jacobian, ANALYSIS_TRAN)
    loads into the caller-bound 2D row buffers; scatters use marshal-
    precomputed *global* row (``node_grow2d``) / flat-Jacobian
    (``jac*_gflat2d``) indices, −1 = dropped (rail / input-driven terminal /
    collapsed node).

    ``dQ/dt`` uses backward-Euler (``method_id=0``) or variable-step BDF2
    (``method_id=1``, BE on the first interval); linear capacitors are
    discretized identically on their branch voltage. Device charge history is
    taken from the last Newton eval of an accepted step (error ~vtol).

    Returns ``(Xhist, nfail, fail_idx)``; ``nfail=-1`` signals an OSDI $fatal
    (caller raises), otherwise nfail counts non-converged steps (the solve
    continues from the best iterate) and fail_idx is the first such step.
    """
    N = tgrid.shape[0]
    ntot = X0.shape[0]
    ndev = inst_a.shape[0]
    n_nodes = nmap2d.shape[1]
    n_jac = jacR_gflat2d.shape[1]
    Xhist = np.zeros((N, ntot))
    Xhist[0] = X0
    X = X0.copy()
    F = np.zeros(ntot)
    Aflat = np.zeros(ntot * ntot)
    A = Aflat.reshape(ntot, ntot)
    redval = np.zeros(n_nodes)
    Qcur = np.zeros((ndev, n_nodes))
    Qh1 = np.zeros((ndev, n_nodes))
    Qh2 = np.zeros((ndev, n_nodes))
    ncap = cap_value.shape[0]
    vab1 = np.zeros(ncap)
    vab2 = np.zeros(ncap)
    nfail = 0
    ...
    # ── device charges + cap branch voltages at X0 (history seed) ───────
    input_now = input_values[:, 0]
    for i in range(ndev):
        if not _osdi_dev_eval_impl(f_ev0, f_lr0, f_lj0, f_lrr0, f_ljr0, handle0,
                                   f_ev1, f_lr1, f_lj1, f_lrr1, f_ljr1, handle1,
                                   lib_a, i,
                                   inst_a, model_a, info_a, residR_ptr_a,
                                   residQ_ptr_a, v2d, residR2d, residQ2d,
                                   jac2d, react2d, nmap2d, term_kind2d,
                                   term_ref2d, term_val2d, nred_a, int_base_a,
                                   T, X, input_now, redval):
            return Xhist, -1, 0
        for j in range(n_nodes):
            Qh1[i, j] = residQ2d[i, j]
            Qh2[i, j] = residQ2d[i, j]
    for e in range(ncap):
        va = _term_value_impl(cap_a_kind[e], cap_a_ref[e], cap_a_val[e],
                              X, input_now)
        vb = _term_value_impl(cap_b_kind[e], cap_b_ref[e], cap_b_val[e],
                              X, input_now)
        vab1[e] = va - vb
        vab2[e] = va - vb
    ...
    for t_idx in range(1, N):
        ...
        for _ in range(newton_maxit):
            for r in range(ntot):
                F[r] = 0.0
            for r in range(ntot * ntot):
                Aflat[r] = 0.0
            # devices: KCL currents + dQ/dt, resistive + a0*reactive Jacobian
            fatal = False
            for i in range(ndev):
                if not _osdi_dev_eval_impl(f_ev0, f_lr0, f_lj0, f_lrr0, f_ljr0,
                                           handle0, f_ev1, f_lr1, f_lj1, f_lrr1,
                                           f_ljr1, handle1, lib_a, i,
                                           inst_a, model_a, info_a,
                                           residR_ptr_a, residQ_ptr_a, v2d,
                                           residR2d, residQ2d, jac2d, react2d,
                                           nmap2d, term_kind2d, term_ref2d,
                                           term_val2d, nred_a, int_base_a,
                                           T, X, input_now, redval):
                    fatal = True
                    break
                for j in range(n_nodes):
                    ...
                    g = node_grow2d[i, j]
                    if g >= 0:
                        F[g] += residR2d[i, j] + a0 * q + a1 * Qh1[i, j] \
                            + a2 * Qh2[i, j]
                for k in range(n_jac):
                    gr = jacR_gflat2d[i, k]
                    if gr >= 0:
                    ...
                    gq = jacQ_gflat2d[i, k]
                    if gq >= 0:
                        Aflat[gq] += a0 * react2d[i, k]
            if fatal:
                return Xhist, -1, t_idx
            _osdi_stamp_elements_impl(
                F, Aflat, X, input_now, a0, a1, a2, vab1, vab2,
                res_a_kind, res_a_ref, res_a_val, res_b_kind, res_b_ref,
                res_b_val, res_ai, res_bi, res_g,
            ...
            if not ok:
                break
            if smax > step_limit:                 # damp large Newton steps
                sc = step_limit / smax
                for r in range(ntot):
                    dx[r] *= sc
                smax = step_limit
        ...
        if not converged:
            ...
            if fail_idx < 0:
                fail_idx = t_idx
        Xhist[t_idx] = X
        # rotate histories (device charges from the last accepted-step eval;
        # cap branch voltages recomputed exactly at the accepted X)
        for i in range(ndev):
            for j in range(n_nodes):
                Qh2[i, j] = Qh1[i, j]
                Qh1[i, j] = Qcur[i, j]
        for e in range(ncap):
            ...
            vab2[e] = vab1[e]
            vab1[e] = va - vb
    return Xhist, nfail, fail_idx


def _osdi_transient_adaptive_impl(
        f_ev0, f_lr0, f_lj0, f_lrr0, f_ljr0, handle0,
        f_ev1, f_lr1, f_lj1, f_lrr1, f_ljr1, handle1, lib_a,
        inst_a, model_a, info_a, residR_ptr_a, residQ_ptr_a,
        v2d, residR2d, residQ2d, jac2d, react2d,
        ...
        n_kcl, n_aug, T,
        newton_maxit, newton_vtol, step_limit, gmin,
        reltol, vabstol, iabstol, max_substeps):
    """Adaptive-step transient for OSDI-device circuits.

    Same global-fold formulation as :func:`_osdi_transient_grid_impl` but with
    error-controlled internal substeps between the requested ``tgrid`` samples
    (variable-step BDF2, BE on the first step): the local truncation error is
    estimated against a linear-extrapolation predictor and each substep is
    accepted when ``max |X - X_pred| / (reltol*|X| + abstol) <= 1`` (voltage
    rows use ``vabstol``, branch-current rows ``iabstol``); rejected steps are
    retried at ``h * max(0.25, 0.9*err^(-1/3))``, accepted ones grow ``h`` by
    up to 2x. Inputs are linearly interpolated between their tgrid columns.
    ``X`` is recorded exactly at every tgrid sample.

    Returns ``(Xhist, nfail, fail_idx, total_substeps)`` — nfail counts
    substeps that had to be force-accepted at the minimum step (Newton or LTE
    failure), -1 signals an OSDI $fatal.
    """
    N = tgrid.shape[0]
    ntot = X0.shape[0]
    ndev = inst_a.shape[0]
    ...
    for i in range(ndev):
        if not _osdi_dev_eval_impl(f_ev0, f_lr0, f_lj0, f_lrr0, f_ljr0, handle0,
                                   ...
                                   jac2d, react2d, nmap2d, term_kind2d,
                                   term_ref2d, term_val2d, nred_a, int_base_a,
                                   T, X, input_sub, redval):
            return Xhist, -1, 0, total_sub
        for j in range(n_nodes):
            Qh1[i, j] = residQ2d[i, j]
            Qh2[i, j] = residQ2d[i, j]
    for e in range(ncap):
    ...
    h_prev = 0.0                      # previous ACCEPTED substep (0 -> BE, no LTE)
    h_next = 0.0
    for t_idx in range(1, N):
        t0 = tgrid[t_idx - 1]
        t1 = tgrid[t_idx]
        ...
        while t < t1 - 1e-15 * (abs(t1) + 1.0):
            ...
            if h < h_min:
                h = h_min
            total_sub += 1
            over_budget = total_sub > max_substeps
            if over_budget:
                h = t1 - t                     # bail out coarsely, flag below
            theta = (t + h - t0) / dt_int
            for r in range(nin):
                input_sub[r] = (input_values[r, t_idx - 1] * (1.0 - theta)
                                + input_values[r, t_idx] * theta)
            if h_prev > 0.0:
                a0 = (2.0 * h + h_prev) / (h * (h + h_prev))
                a1 = -(h + h_prev) / (h * h_prev)
                a2 = h / (h_prev * (h + h_prev))
            else:                              # BE startup
                a0 = 1.0 / h
                a1 = -1.0 / h
                a2 = 0.0
            ...
            for _ in range(newton_maxit):
                ...
                for r in range(ntot * ntot):
                    Aflat[r] = 0.0
                for i in range(ndev):
                    if not _osdi_dev_eval_impl(
                            f_ev0, f_lr0, f_lj0, f_lrr0, f_ljr0, handle0,
                            f_ev1, f_lr1, f_lj1, f_lrr1, f_ljr1, handle1,
                            lib_a, i, inst_a, model_a, info_a,
                            residR_ptr_a, residQ_ptr_a, v2d,
                            residR2d, residQ2d, jac2d, react2d,
                            nmap2d, term_kind2d, term_ref2d, term_val2d,
                            nred_a, int_base_a, T, X, input_sub, redval):
                        fatal = True
                        break
                    for j in range(n_nodes):
                        q = residQ2d[i, j]
                    ...
                    for k in range(n_jac):
                        ...
                        if gq >= 0:
                            Aflat[gq] += a0 * react2d[i, k]
                if fatal:
                    return Xhist, -1, t_idx, total_sub
                _osdi_stamp_elements_impl(
                    F, Aflat, X, input_sub, a0, a1, a2, vab1, vab2,
                    res_a_kind, res_a_ref, res_a_val, res_b_kind, res_b_ref,
                    res_b_val, res_ai, res_bi, res_g,
                    cap_a_kind, cap_a_ref, cap_a_val, cap_b_kind, cap_b_ref,
                    cap_b_val, cap_ai, cap_bi, cap_value,
                ...
                if smax < newton_vtol:
                    converged = True
                    break
            forced = h <= h_min * 1.0000001 or over_budget
            if not converged and not forced:
                for r in range(ntot):
                    X[r] = Xsave[r]
                h_next = 0.5 * h
                continue
            if converged and h_prev > 0.0 and not forced:
                # LTE vs the linear-extrapolation predictor
                err = 0.0
                ratio = h / h_prev
                for r in range(ntot):
                    xp = Xsave[r] + (Xsave[r] - X_prev[r]) * ratio
                    if n_kcl <= r < n_aug:
                        tol = reltol * abs(X[r]) + iabstol
                    else:
                        tol = reltol * abs(X[r]) + vabstol
                    e_r = abs(X[r] - xp) / tol
                    if e_r > err:
                        err = e_r
                if err > 1.0:
                ...
                h_next = h * 2.0
            if not converged:
                nfail += 1                     # force-accepted at minimum step
                if fail_idx < 0:
                    fail_idx = t_idx
            # accept the substep
            for r in range(ntot):
            ...
            for e in range(ncap):
                va = _term_value_impl(cap_a_kind[e], cap_a_ref[e],
                                      cap_a_val[e], X, input_sub)
                vb = _term_value_impl(cap_b_kind[e], cap_b_ref[e],
                                      cap_b_val[e], X, input_sub)
                vab2[e] = vab1[e]
                vab1[e] = va - vb
        Xhist[t_idx] = X
    return Xhist, nfail, fail_idx, total_sub
...
if NUMBA_AVAILABLE:
    ...
    _osdi_newton_step_impl = njit(cache=NUMBA_CACHE)(_osdi_newton_step_impl)
    _osdi_schur_impl = njit(cache=NUMBA_CACHE)(_osdi_schur_impl)
